...
function renderReviews(reviewsPromise) {
    reviewsPromise.then(arr => {
        const markup = arr.map(({ avatar_url, _id, author, review }) => {
            ...
            const textClass = isTextOverflowing ? 'review-user-comment-collapsed' : '';
            const trimmedReview = isTextOverflowing ? review.slice(0, maxTextLength) + '...' : review;
            ...
            return `<li class="review-carusel-list-item">
                <div class="user-review-container">
                    ...
                    <div class="user-data-review-container" data-review="${review}">
                        ...
                        ${isTextOverflowing ? '<button class="toggle-btn">Згорнути</button>' : ''}
                    </div>
                </div>
            </li>`;
        }).join("");
        ...
        buttons.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                const comment = commentContainer.querySelector('.review-user-comment');
